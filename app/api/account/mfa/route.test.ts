  ...
  it('returns MFA status flags', async () => {
    ...
    prismaMock.user.findUnique.mockResolvedValue(
      createUser({
        ...
        emailEncrypted: Buffer.alloc(1, 0x01),
      }),
    )
    const res = await GET(createNextRequest('http://localhost/api/account/mfa'))
...
describe('POST /api/account/mfa', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await POST(
      createNextRequest('http://localhost/api/account/mfa', {
        method: 'POST',
        body: { method: 'totp', action: 'setup' },
      }),
    )
    expect(res.status).toBe(401)
  })
  ...
  describe('TOTP confirm', () => {
    it('returns 400 when setup expired', async () => {
      ...
      const res = await POST(
        createNextRequest('http://localhost/api/account/mfa', {
          method: 'POST',
          body: { method: 'totp', action: 'confirm', code: '123456' },
        }),
      )
    ...
    it('returns 401 for invalid code', async () => {
      ...
      const res = await POST(
        createNextRequest('http://localhost/api/account/mfa', {
          method: 'POST',
          body: { method: 'totp', action: 'confirm', code: '000000' },
        }),
      )
      expect(res.status).toBe(401)
    })
  ...
  describe('TOTP disable', () => {
    it('disables TOTP and clears fields', async () => {
      ...
      const res = await POST(
        createNextRequest('http://localhost/api/account/mfa', {
          method: 'POST',
          body: { method: 'totp', action: 'disable' },
        }),
      )
  ...
  describe('Email enable', () => {
    it('requires verified email', async () => {
      mockAuth()
      prismaMock.user.findUnique.mockResolvedValue(
        createUser({ emailVerified: false, emailEncrypted: null }),
      )
      const res = await POST(
        createNextRequest('http://localhost/api/account/mfa', {
          method: 'POST',
          body: { method: 'email', action: 'enable' },
        }),
      )
      expect(res.status).toBe(400)
    })
    ...
    it('enables email 2FA', async () => {
      mockAuth()
      prismaMock.user.findUnique.mockResolvedValue(
        createUser({ emailVerified: true, emailEncrypted: Buffer.alloc(1) }),
      )
      const res = await POST(
        createNextRequest('http://localhost/api/account/mfa', {
          method: 'POST',
  ...
  describe('Passkey disable', () => {
    it('removes all passkeys', async () => {
      ...
      const res = await POST(
        createNextRequest('http://localhost/api/account/mfa', {
          method: 'POST',
          body: { method: 'passkey', action: 'disable' },
        }),
      )
      expect(res.status).toBe(200)
      expect(prismaMock.passkeyCredential.deleteMany).toHaveBeenCalledWith({
        where: { userId: 'user-1' },
      })
    })
  })
  ...
  it('returns 400 for unknown method/action', async () => {
    ...
    const res = await POST(
      createNextRequest('http://localhost/api/account/mfa', {
        method: 'POST',
        body: { method: 'totp', action: 'unknown' },
      }),
    )
    expect(res.status).toBe(400)
  })
