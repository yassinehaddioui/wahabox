...
describe('DELETE /api/account/mfa/passkeys/[id]', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await DELETE(createNextRequest('http://localhost/api/account/mfa/passkeys/cred-1', { method: 'DELETE' }), createRouteContext({ id: 'cred-1' }))
  ...
  it('prevents cross-user deletion', async () => {
    mockAuth()
    prismaMock.passkeyCredential.findFirst.mockResolvedValue(null)
    const res = await DELETE(createNextRequest('http://localhost/api/account/mfa/passkeys/cred-other', { method: 'DELETE' }), createRouteContext({ id: 'cred-other' }))
    expect(res.status).toBe(404)
    expect(prismaMock.passkeyCredential.findFirst).toHaveBeenCalledWith({ where: { id: 'cred-other', userId: 'user-1' } })
  })

  it('deletes and disables flag when last passkey', async () => {
    mockAuth()
    prismaMock.passkeyCredential.findFirst.mockResolvedValue(createPasskeyCredential({ id: 'cred-1' }))
    prismaMock.passkeyCredential.count.mockResolvedValue(0)
    const res = await DELETE(createNextRequest('http://localhost/api/account/mfa/passkeys/cred-1', { method: 'DELETE' }), createRouteContext({ id: 'cred-1' }))
    expect((await res.json()).data.deleted).toBe(true)
    expect(prismaMock.user.update).toHaveBeenCalledWith({ where: { id: 'user-1' }, data: { mfaPasskey: false } })
  })
