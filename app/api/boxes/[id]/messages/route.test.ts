...
describe('GET /api/boxes/[id]/messages', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await GET(
      createNextRequest('http://localhost/api/boxes/b1/messages'),
  ...
  it('returns 404 when box not found or not owned', async () => {
    ...
    const res = await GET(
      createNextRequest('http://localhost/api/boxes/other/messages'),
  ...
  it('returns messages with base64 ciphertext, ordered desc', async () => {
    ...
    const newer = createMessage({
      ...
      readAt: new Date('2025-01-02T12:00:00.000Z'),
    })
