...
describe('PATCH /api/boxes/[id]', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await PATCH(createNextRequest('http://localhost/api/boxes/b1', { method: 'PATCH', body: {} }), createRouteContext({ id: 'b1' }))
  ...
  it('returns 400 for invalid CSRF', async () => {
    ...
    const res = await PATCH(createNextRequest('http://localhost/api/boxes/b1', { method: 'PATCH', body: { csrfToken: 'bad' } }), createRouteContext({ id: 'b1' }))
  ...
  it('enforces ownership returns 404 for another user box', async () => {
    ...
    const res = await PATCH(createNextRequest('http://localhost/api/boxes/other', { method: 'PATCH', body: { label: 'H', csrfToken: 'v' } }), createRouteContext({ id: 'other' }))
  ...
  it('updates box fields', async () => {
    ...
    prismaMock.poBox.update.mockResolvedValue(createPoBox({ id: 'b1', label: 'Updated', greeting: 'Hi', slug: 'new-slug' }) as any)
    const res = await PATCH(createNextRequest('http://localhost/api/boxes/b1', { method: 'PATCH', body: { label: 'Updated', greeting: 'Hi', rotateSlug: true, csrfToken: 'v' } }), createRouteContext({ id: 'b1' }))
  ...
  it('rotates slug', async () => {
    ...
    const res = await PATCH(createNextRequest('http://localhost/api/boxes/b1', { method: 'PATCH', body: { rotateSlug: true, csrfToken: 'v' } }), createRouteContext({ id: 'b1' }))
    const body = await res.json()
  ...
  it('removes password when null', async () => {
    mockAuth()
    vi.mocked(verifyAndConsumeCsrfToken).mockResolvedValue(true)
    prismaMock.poBox.findFirst.mockResolvedValue(createPoBox({ id: 'b1', passwordHash: '$2a$12$hash' }))
    prismaMock.poBox.update.mockResolvedValue(createPoBox({ id: 'b1', passwordHash: null }) as any)
    const res = await PATCH(createNextRequest('http://localhost/api/boxes/b1', { method: 'PATCH', body: { password: null, csrfToken: 'v' } }), createRouteContext({ id: 'b1' }))
...
describe('DELETE /api/boxes/[id]', () => {
  ...
  it('enforces ownership', async () => {
    ...
    vi.mocked(verifyAndConsumeCsrfToken).mockResolvedValue(true)
    prismaMock.poBox.findFirst.mockResolvedValue(null)
    const res = await DELETE(createNextRequest('http://localhost/api/boxes/other', { method: 'DELETE', body: { csrfToken: 'v' } }), createRouteContext({ id: 'other' }))
    expect(res.status).toBe(404)
  })
  ...
  it('deletes owned box', async () => {
    ...
    const res = await DELETE(createNextRequest('http://localhost/api/boxes/b1', { method: 'DELETE', body: { csrfToken: 'v' } }), createRouteContext({ id: 'b1' }))
